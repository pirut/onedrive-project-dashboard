...
async function listAsyncJobs(dataverse: DataverseClient, stepId: string, { since, limit }: { since: string | null; limit: number }) {
    const filters = [`_sdkmessageprocessingstepid_value eq ${stepId}`];
    if (since) {
...
export async function GET(request: Request) {
    ...
    const minutes = parseNumber(url.searchParams.get("minutes"), 120);
    const limit = Math.max(1, parseNumber(url.searchParams.get("limit"), 50));
    const perStepLimit = Math.max(1, Math.ceil(limit / 3));
    ...
    try {
        ...
        for (const endpoint of endpoints) {
            ...
            for (const step of steps as Array<Record<string, unknown>>) {
                const stepId = step.sdkmessageprocessingstepid as string | undefined;
        ...
        logger.error("Debug Dataverse webhook jobs failed", { error: (error as Error)?.message || String(error) });
