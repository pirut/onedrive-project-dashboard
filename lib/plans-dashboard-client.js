(function(){
  ...
  var bcUnlinkBtn = document.getElementById('bc-projects-unlink');
  ...
  function updateBcSelectedCounts(){
    ...
    if(bcUnlinkBtn) bcUnlinkBtn.disabled = bcSelected.size === 0;
  }
  ...
  function renderBcProjects(){
    ...
    if(bcTbody){
      var rows = bcFiltered.map(function(item){
        ...
        var syncable = Number(item.tasksSyncable || 0);
        var staleLinks = Number(item.tasksStaleLinks || 0);
        var tasksTitleParts = [];
        if(syncable){ tasksTitleParts.push('syncable in BC: ' + syncable); }
        if(staleLinks){ tasksTitleParts.push('stale BC links: ' + staleLinks); }
        var tasksTitle = tasksTitleParts.join(' | ');
        ...
        return '<tr>'
          ...
          + '<td class="small mono"' + (tasksTitle ? (' title="' + htmlEscape(tasksTitle) + '"') : '') + '>' + htmlEscape(tasksText) + '</td>'
          + '<td>' + lastSync + '</td>'
  ...
  async function clearBcLinks(projectNos){
    var data = await runPlannerAssetsAction({
      action: 'clear-bc-links',
      projectNos: projectNos
    });
    return data;
  }
  ...
  if(bcUnlinkBtn){
    bcUnlinkBtn.addEventListener('click', async function(){
      if(!bcSelected.size){
        setBcStatus('Select at least one BC project to unlink.', 'warn');
        return;
      }
      var ok = window.confirm('Unlink selected BC projects? This clears planner IDs on every task so you can restart sync.');
      if(!ok) return;
      setBcStatus('Unlinking selected BC projects…', 'warn');
      try{
        var projectNos = Array.from(bcSelected);
        var data = await clearBcLinks(projectNos);
        renderBcOutput(data);
        await fetchAssets();
        setBcStatus('Unlink complete. You can now restart sync.', 'ok');
      }catch(err){
        renderBcOutput(err && err.message ? err.message : err);
        setBcStatus('Unlink failed: ' + (err && err.message ? err.message : 'error'), 'bad');
      }
    });
  }
  ...
  if(resetBtn){
    resetBtn.addEventListener('click', async function(){
      ...
      try{
        ...
        var bcProjectNos = bcProjects.map(function(item){ return item.projectNo; }).filter(Boolean);
        var unlinkResult = bcProjectNos.length ? await clearBcLinks(bcProjectNos) : { ok: true, results: [] };
        var recreateResult = await recreateAll();
        await fetchAssets();
        var summary = {
          deleteGraph: graphResult,
          deletePremium: premiumResult,
          clearLinks: unlinkResult,
