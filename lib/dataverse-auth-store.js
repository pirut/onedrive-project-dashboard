...
export async function saveDataverseAuthState(state, codeVerifier) {
...
export async function consumeDataverseAuthState(state) {
    if (!state) return "";
    const store = await readStore();
    const authState = store && store.authState ? store.authState : null;
    if (!authState || !authState.state || authState.state !== state) return "";
