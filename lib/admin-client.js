    ...
    (function(){
      ...
      var plannerRunFullSyncBtn = document.getElementById('planner-run-full-sync');
      ...
      function renderPlannerProjects(){
        ...
        var rows = filtered.map(function(item){
          ...
          return '<tr>'
            ...
            + '<td>' + syncBadge + '</td>'
            + '<td style="display:flex;gap:6px;flex-wrap:wrap">' + syncBtn + toggleBtn + '</td>'
            + '</tr>';
        }).join('');
      ...
      async function bulkSyncProjects(){
        ...
        var list = Array.from(plannerProjectsSelected);
        setPlannerProjectsStatus('Running BC → Planner sync for ' + list.length + ' projects…', 'muted');
        var failed = [];
        for(var i = 0; i < list.length; i++){
          var projectNo = list[i];
          try{
            await runBcSync(projectNo);
            logPlannerEvent('BC → Planner sync (' + projectNo + ') complete', 'ok');
          }catch(err){
            var msg = err && err.message ? err.message : 'error';
            failed.push(projectNo + ': ' + msg);
            logPlannerEvent('BC → Planner sync (' + projectNo + ') failed: ' + msg, 'bad');
          }
        }
        await fetchPlannerProjects();
        if(failed.length){
      ...
      if(plannerRunFullSyncBtn){
        plannerRunFullSyncBtn.addEventListener('click', function(){
          var projectNo = plannerProjectInput && plannerProjectInput.value ? plannerProjectInput.value.trim() : '';
          var body = projectNo ? { projectNo: projectNo, includePlanner: true } : { includePlanner: true };
          runPlannerRequest({ label: 'Full sync', url: '/api/sync/run-bc-to-planner', body: body });
        });
      }
        ...
        plannerProjectsTbody.addEventListener('click', async function(ev){
          ...
          if(action === 'sync-project'){
            await runPlannerRequest({
              label: 'BC → Planner sync (' + projectNo + ')',
