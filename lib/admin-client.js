    ...
    (function(){
      ...
      var plannerPlanMaintenanceBtn = document.getElementById('planner-plan-maintenance');
      var plannerArchivedPlansBtn = document.getElementById('planner-archived-plans');
      ...
      var plannerSubscriptionsListBtn = document.getElementById('planner-subscriptions-list');
      ...
      var plannerTestGraphBtn = document.getElementById('planner-test-graph');
      ...
      var plannerOrphanPlansEl = document.getElementById('planner-orphan-plans');
      ...
      function renderPlannerProjects(){
        ...
        var rows = filtered.map(function(item){
          var planLabel = item.planTitle || item.planId || '—';
          var planLink = item.planUrl ? '<a href="' + htmlEscape(item.planUrl) + '" target="_blank" rel="noreferrer">' + htmlEscape(planLabel) + '</a>' : htmlEscape(planLabel);
          var planMeta = item.planId ? ('<div class="small mono">' + htmlEscape(item.planId) + '</div>') : '';
          var planStatus = item.planExists ? '<span class="badge ok">Plan found</span>' : '<span class="badge warn">Missing</span>';
          var planLinkNote = item.planLinked ? '' : (item.planId ? '<div class="small muted">Detected by title</div>' : '');
          ...
          var deleteBtn = item.planId ? '<button type="button" data-action="delete-plan" data-project-no="' + htmlEscape(item.projectNo) + '" data-plan-id="' + htmlEscape(item.planId) + '" style="background:#ef4444;color:#fff;border:0;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:12px">Delete plan</button>' : '';
          ...
          return '<tr>'
            ...
            + '<td>' + planLink + planMeta + planLinkNote + '<div class="small" style="margin-top:4px">' + planStatus + '</div></td>'
            + '<td>' + lastSyncCell + '</td>'
            + '<td>' + syncBadge + '</td>'
            + '<td style="display:flex;gap:6px;flex-wrap:wrap">' + syncBtn + toggleBtn + deleteBtn + '</td>'
            + '</tr>';
        }).join('');
        plannerProjectsTbody.innerHTML = rows || '<tr><td colspan="7" class="muted">No projects found.</td></tr>';
        if(plannerOrphanPlansEl){
          if(Array.isArray(plannerOrphanCache) && plannerOrphanCache.length){
            var orphanLinks = plannerOrphanCache.map(function(plan){
              var label = plan.title || plan.planId || 'Plan';
              if(plan.planUrl){
                return '<a href="' + htmlEscape(plan.planUrl) + '" target="_blank" rel="noreferrer">' + htmlEscape(label) + '</a>';
              }
              return htmlEscape(label);
            }).join(', ');
            plannerOrphanPlansEl.innerHTML = '<span class="muted">Planner-only plans:</span> ' + orphanLinks;
          } else {
            plannerOrphanPlansEl.textContent = 'Planner-only plans: none';
          }
        }
      ...
      async function fetchPlannerProjects(){
        if(!plannerProjectsTbody) return;
        setPlannerProjectsStatus('Loading planner projects…', 'muted');
        try{
          ...
          plannerOrphanCache = Array.isArray(payload.orphanPlans) ? payload.orphanPlans : [];
          ...
          renderPlannerProjects();
          setPlannerProjectsStatus('Failed to load planner projects: ' + (err && err.message ? err.message : 'error'), 'bad');
        }
      }
      ...
      async function deleteProjectPlan(projectNo, planId){
        if(!projectNo) return;
        var confirmed = window.confirm('Delete the Planner plan for ' + projectNo + '? This disables sync and clears Planner links in BC.');
        if(!confirmed) return;
        setPlannerProjectsStatus('Deleting plan for ' + projectNo + '…', 'muted');
        try{
          var res = await fetch('/api/sync/projects', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'deletePlan', projectNo: projectNo, planId: planId || '' })
          });
          var payload = await res.json();
          if(!res.ok || (payload && payload.ok === false)){
            var msg = payload && payload.error ? payload.error : ('HTTP ' + res.status);
          ...
          var warning = false;
          if(payload && payload.planDelete && payload.planDelete.attempted && payload.planDelete.ok === false) warning = true;
          if(payload && payload.clearedTasks && payload.clearedTasks.failed > 0) warning = true;
          renderPlannerOutput(payload);
          await fetchPlannerProjects();
          if(warning){
            setPlannerProjectsStatus('Plan delete completed with warnings for ' + projectNo + '.', 'warn');
            logPlannerEvent('Plan delete completed with warnings for ' + projectNo, 'warn');
          } else {
            setPlannerProjectsStatus('Plan deleted for ' + projectNo + '.', 'ok');
            logPlannerEvent('Plan deleted for ' + projectNo, 'ok');
          }
        }catch(err){
          var msg = err && err.message ? err.message : 'error';
          renderPlannerOutput({ error: msg });
          setPlannerProjectsStatus('Plan delete failed for ' + projectNo + ': ' + msg, 'bad');
          logPlannerEvent('Plan delete failed for ' + projectNo + ': ' + msg, 'bad');
        }
      }
      ...
      async function runBcSync(projectNo){
        var res = await fetch('/api/sync/run-bc-to-planner', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if(plannerRunBcBtn){
        plannerRunBcBtn.addEventListener('click', function(){
          ...
          runPlannerRequest({ label: 'Sync', url: '/api/sync/run-bc-to-planner', body: body });
        });
      }
      if(plannerRunBcPrBtn){
        plannerRunBcPrBtn.addEventListener('click', function(){
          runPlannerRequest({ label: 'Sync (PR00001)', url: '/api/sync/run-bc-to-planner', body: { projectNo: 'PR00001' } });
        });
      }
      if(plannerPlanMaintenanceBtn){
        plannerPlanMaintenanceBtn.addEventListener('click', function(){
          var projectNo = plannerProjectInput && plannerProjectInput.value ? plannerProjectInput.value.trim() : '';
          if(!projectNo){
            var proceed = window.confirm('Run plan maintenance for ALL projects?\n\nThis will delete duplicate plans and update titles.');
            if(!proceed) return;
          }
          var body = projectNo ? { projectNo: projectNo } : undefined;
          runPlannerRequest({ label: 'Plan maintenance', url: '/api/sync/plan-maintenance', body: body });
        });
      }
      if(plannerArchivedPlansBtn){
        plannerArchivedPlansBtn.addEventListener('click', function(){
          var deep = window.confirm('Use deep scan? This fetches each plan to detect archived state (slower).\n\nOK = deep scan, Cancel = quick scan.');
          var url = deep ? '/api/sync/archived-plans?deep=1' : '/api/sync/archived-plans';
          runPlannerRequest({ label: 'List archived plans', url: url, method: 'GET' });
        });
      }
      ...
      if(plannerDebugDecisionBtn){
        plannerDebugDecisionBtn.addEventListener('click', function(){
          ...
          runPlannerRequest({
            ...
            url: '/api/sync/debug-bc-task?includePlanner=1',
            body: { projectNo: projectNo, taskNo: taskNo }
          });
        });
      }
      ...
      if(plannerSyncLogLoadBtn){
        plannerSyncLogLoadBtn.addEventListener('click', async function(){
          setPlannerSyncLogStatus('Loading planner logs...', 'muted');
          renderPlannerSyncLog(null);
          try{
            var res = await fetch('/api/sync/planner-log?limit=200', {
      ...
      if(plannerSubscriptionsListBtn){
        plannerSubscriptionsListBtn.addEventListener('click', async function(){
          setPlannerStatus('Loading Graph subscriptions…', 'muted');
          renderWebhookOutput(null);
          try{
            var res = await fetch('/api/sync/subscriptions/list', {
              headers: { 'cache-control': 'no-cache' }
            });
            var payload = await res.json();
            if(!res.ok || (payload && payload.ok === false)){
            ...
            renderWebhookOutput(payload);
            setPlannerStatus('Graph subscriptions loaded.', 'ok');
            logPlannerEvent('Graph subscriptions loaded', 'ok');
          }catch(err){
            setPlannerStatus('Subscriptions list failed: ' + (err && err.message ? err.message : 'error'), 'bad');
            renderWebhookOutput({ error: err && err.message ? err.message : String(err) });
            logPlannerEvent('Subscriptions list failed: ' + (err && err.message ? err.message : 'error'), 'bad');
          }
        });
      }
      ...
      if(plannerTestGraphBtn){
        plannerTestGraphBtn.addEventListener('click', function(){
          runPlannerRequest({ label: 'Planner API test', url: '/api/sync/planner-test', method: 'GET' });
        });
      }
      ...
      if(action === 'planner-test' && plannerTestGraphBtn){ plannerTestGraphBtn.click(); return; }
        ...
        plannerProjectsTbody.addEventListener('click', async function(ev){
          ...
          if(action === 'sync-project'){
            await runPlannerRequest({
              ...
              url: '/api/sync/run-bc-to-planner',
          ...
          if(action === 'delete-plan'){
            ...
            await deleteProjectPlan(projectNo, planId);
            return;
          }
        });
