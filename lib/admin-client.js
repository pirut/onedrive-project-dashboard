    ...
    (function(){
      ...
      var plannerRunBcReseedBtn = document.getElementById('planner-run-bc-reseed');
      ...
      var plannerRunAutoBtn = document.getElementById('planner-run-auto');
      ...
      var plannerProjectsBulkShareBtn = document.getElementById('planner-projects-bulk-share');
      ...
      function updatePlannerProjectsCount(){
        ...
        if(plannerProjectsBulkShareBtn) plannerProjectsBulkShareBtn.disabled = disabled;
      ...
      function renderPlannerProjects(){
        ...
        var rows = filtered.map(function(item){
          ...
          var shareBtn = item.premiumProjectId ? '<button type="button" data-action="share-access" data-project-no="' + htmlEscape(item.projectNo) + '" data-premium-project-id="' + htmlEscape(item.premiumProjectId) + '" style="background:#2b61d1;color:#fff;border:0;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:12px">Share team</button>' : '';
          ...
          return '<tr>'
            ...
            + '<td>' + lastSyncCell + '</td>'
            + '<td>' + syncBadge + '</td>'
            + '<td style="display:flex;gap:6px;flex-wrap:wrap">' + syncBtn + shareBtn + toggleBtn + clearBtn + '</td>'
            + '</tr>';
        }).join('');
      ...
      function renderPlannerAssets(){
        ...
        if(plannerAssetsDvTbody){
          var dvRows = dvFiltered.map(function(item){
            ...
            var linkCell = item.url ? ('<a href="' + htmlEscape(item.url) + '" target="_blank" rel="noreferrer" class="badge">Open</a>') : '<span class="muted">—</span>';
            return '<tr>'
              + '<td>' + selectBox + '</td>'
              + '<td>' + htmlEscape(item.title || '') + '</td>'
          ...
          plannerAssetsDvTbody.innerHTML = dvRows || '<tr><td colspan="5" class="muted">No Premium projects found.</td></tr>';
        }
      ...
      async function applyProjectShare(projectNo, premiumProjectId){
        var res = await fetch('/api/sync/projects', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      async function shareProjectAccess(projectNo, premiumProjectId){
        if(!projectNo || !premiumProjectId){
          setPlannerProjectsStatus('Share skipped: project is missing a Premium project ID.', 'warn');
          return;
        }
        setPlannerProjectsStatus('Sharing ' + projectNo + ' with team…', 'muted');
        try{
          var payload = await applyProjectShare(projectNo, premiumProjectId);
          renderPlannerOutput(payload);
          setPlannerProjectsStatus('Shared ' + projectNo + ' with configured team access.', 'ok');
          logPlannerEvent('Shared team access for ' + projectNo, 'ok');
        }catch(err){
          var msg = err && err.message ? err.message : 'error';
          renderPlannerOutput({ error: msg });
          setPlannerProjectsStatus('Share failed for ' + projectNo + ': ' + msg, 'bad');
          logPlannerEvent('Share failed for ' + projectNo + ': ' + msg, 'bad');
        }
      }
      async function bulkShareProjectAccess(){
        if(!plannerProjectsSelected.size){
          setPlannerProjectsStatus('Select at least one project to share.', 'warn');
          return;
        }
        var list = Array.from(plannerProjectsSelected);
        var mapByProject = new Map(plannerProjectsCache.map(function(item){ return [item.projectNo, item]; }));
        setPlannerProjectsStatus('Sharing team access for ' + list.length + ' projects…', 'muted');
        var skipped = [];
        var failed = [];
        for(var i = 0; i < list.length; i++){
          var projectNo = list[i];
          var item = mapByProject.get(projectNo);
          var premiumProjectId = item && item.premiumProjectId ? String(item.premiumProjectId) : '';
          if(!premiumProjectId){
            skipped.push(projectNo);
            continue;
          }
          try{
            await applyProjectShare(projectNo, premiumProjectId);
            logPlannerEvent('Shared team access (' + projectNo + ')', 'ok');
          }catch(err){
            var msg = err && err.message ? err.message : 'error';
            failed.push(projectNo + ': ' + msg);
            logPlannerEvent('Share failed (' + projectNo + '): ' + msg, 'bad');
          }
        }
        if(failed.length){
          setPlannerProjectsStatus('Bulk share completed with ' + failed.length + ' failures' + (skipped.length ? (' and ' + skipped.length + ' skipped') : '') + '.', 'warn');
          return;
        }
        if(skipped.length){
          setPlannerProjectsStatus('Bulk share completed; skipped ' + skipped.length + ' projects without Premium IDs.', 'warn');
          return;
        }
        setPlannerProjectsStatus('Bulk share complete for ' + list.length + ' projects.', 'ok');
      }
      ...
      if(plannerRunBcBtn){
        plannerRunBcBtn.addEventListener('click', function(){
          ...
          var body = { includePremiumChanges: false };
          if(projectNo){ body.projectNo = projectNo; }
          runPlannerRequest({ label: 'BC → Premium Sync', url: '/api/sync/bc-to-premium', body: body });
        });
      }
      if(plannerRunBcReseedBtn){
        plannerRunBcReseedBtn.addEventListener('click', function(){
          var projectNo = plannerProjectInput && plannerProjectInput.value ? plannerProjectInput.value.trim() : '';
          if(!projectNo){
            setPlannerStatus('Enter Project No before forcing task recreation.', 'warn');
            logPlannerEvent('Missing Project No for force recreate.', 'warn');
            return;
          }
          var confirmed = window.confirm('Force recreate tasks for ' + projectNo + '? This ignores stored Planner task IDs and creates new Planner tasks.');
          if(!confirmed) return;
          runPlannerRequest({
            label: 'BC → Premium Force Recreate (' + projectNo + ')',
            url: '/api/sync/bc-to-premium-reseed',
            body: { projectNo: projectNo, includePremiumChanges: false }
          });
        });
      }
      if(plannerRunBcPrBtn){
        plannerRunBcPrBtn.addEventListener('click', function(){
          runPlannerRequest({ label: 'BC → Premium Sync (PR00001)', url: '/api/sync/bc-to-premium', body: { projectNo: 'PR00001', includePremiumChanges: false } });
        });
      }
      if(plannerPollPremiumBtn){
        plannerPollPremiumBtn.addEventListener('click', function(){
          runPlannerRequest({ label: 'Premium → BC Sync', url: '/api/sync/premium-to-bc', method: 'POST' });
        });
      }
      if(plannerRunAutoBtn){
        plannerRunAutoBtn.addEventListener('click', function(){
          runPlannerRequest({ label: 'Auto Sync', url: '/api/sync/auto', method: 'POST' });
        });
      }
      ...
      if(bcSubsRenewBtn){
        bcSubsRenewBtn.addEventListener('click', function(){
          ...
          var body = {};
          if(entitySets.length) body.entitySets = entitySets;
          runBcRequest({ label: 'Renew BC subscriptions', url: '/api/sync/bc-subscriptions/renew', body: Object.keys(body).length ? body : undefined });
        });
      }
      if(bcSubsDeleteBtn){
      ...
      if(bcJobsProcessBtn){
        bcJobsProcessBtn.addEventListener('click', function(){
          runBcRequest({ label: 'Process BC jobs', url: '/api/sync/bc-jobs/process', method: 'POST' });
        });
      }
      if(bcWebhookLogBtn){
      ...
      if(plannerProjectsBulkShareBtn){
        plannerProjectsBulkShareBtn.addEventListener('click', function(){
          bulkShareProjectAccess();
        });
      }
      ...
      if(plannerProjectsTbody){
        plannerProjectsTbody.addEventListener('click', async function(ev){
          ...
          var premiumProjectId = button.getAttribute('data-premium-project-id') || '';
          ...
          if(action === 'share-access'){
            await shareProjectAccess(projectNo, premiumProjectId);
            return;
          }
