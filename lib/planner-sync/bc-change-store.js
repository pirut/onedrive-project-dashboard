...
function normalizeScopes(raw) {
    ...
    for (const [key, value] of Object.entries(source || {})) {
        if (value && typeof value === "object" && typeof value.lastSeq === "number") {
...
async function writeFileStore(payload) {
    try {
        ...
        logger.warn("Failed to write BC project change store", { error: error?.message || String(error) });
    }
}
...
export async function getBcProjectChangeCursor(scopeKey) {
...
export async function saveBcProjectChangeCursor(scopeKey, lastSeq) {
    if (!Number.isFinite(lastSeq)) {
