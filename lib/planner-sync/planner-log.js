...
const inMemoryLog = [];
const emitter = globalThis.__plannerLogEmitter || new EventEmitter();

if (!globalThis.__plannerLogEmitter) {
    globalThis.__plannerLogEmitter = emitter;
}
...
export async function appendPlannerLog(entry) {
    ...
    if (redis) {
        try {
            ...
            console.warn("Planner log write failed; using memory log", {
...
export async function listPlannerLog(limit = 50) {
    ...
    if (redis) {
        try {
            ...
            // eslint-disable-next-line no-console
            console.warn("Planner log read failed; using memory log", {
                error: error?.message || String(error),
            });
        }
    }
