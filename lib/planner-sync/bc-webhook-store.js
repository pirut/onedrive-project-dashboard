...
function normalizeEntitySet(entitySet) {
...
async function writeFileStore(store) {
    try {
        ...
        logger.warn("Failed to write BC webhook store", { error: error?.message || String(error) });
    }
}

function buildDedupeKey(job, receivedMs) {
...
export async function saveBcSubscription(entitySet, subscription) {
    const normalized = normalizeEntitySet(entitySet);
...
export async function deleteBcSubscription(entitySet) {
    const normalized = normalizeEntitySet(entitySet);
    if (!normalized) return;
    const redis = getRedis({ requireWrite: true });
    if (redis) {
...
export async function enqueueBcJobs(jobs) {
    ...
    if (redis) {
        for (const raw of jobs) {
            ...
            try {
                ...
                logger.warn("BC webhook enqueue failed", { error: error?.message || String(error) });
            }
        }
...
export async function popBcJobs(maxJobs = 25) {
    const redis = getRedis({ requireWrite: true });
    if (redis) {
        const jobs = [];
...
export async function acquireBcJobLock(ttlSeconds = LOCK_TTL_SECONDS) {
    ...
    if (redis) {
        try {
            ...
            logger.warn("BC webhook lock failed", { error: error?.message || String(error) });
            return null;
        }
    }
...
export async function releaseBcJobLock(lockValue) {
    if (!lockValue) return;
    const redis = getRedis({ requireWrite: true });
    if (redis) {
        try {
            ...
            logger.warn("BC webhook lock release failed", { error: error?.message || String(error) });
            return;
        }
    }
